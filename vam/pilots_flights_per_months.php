...
group by gvauser_id,mes order by ye asc,mes asc,num_per_month desc";
	if (!$result = $db->query($sql)) {
		die('There was an error running the query [' . $db->error . ']');
	}
	echo '<table id="pilots_flights_per_month" class="table table-hover">';
	echo '<thead>';
	echo '<tr><th>' . STATISTICS_CALLSIGN . '</th><th>' . STATISTICS_PILOT . '</th><th>' . ST_YEAR . '</th><th>' . ST_MONTH . '</th><th>' . ST_NUMFLIGHTS . '</th></tr>';
	echo '</thead>';
